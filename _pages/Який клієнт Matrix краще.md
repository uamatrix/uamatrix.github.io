...
- приємний вигляд;
- є функції спойлера, ніліпки
- швидкого доступу до пошуку.
...
елемент найтлі на десктопі - красивий, функціональний, все підтримує, швидкість роботи влаштовує
...
елемент ікс теж нарешті виглядає професійно, але ще сируватий

як тільки в іксі буде маркер прочитаного, перехід по реплаю, спейси і треди, то рекомендоватиму знайомим не-технарям
...
флаффі красивий і повільний, що там по фічах окрім просто чатів я не дуже розбирався, колись дзвінки були дивним чином зроблені, зараз хз

і ще флаффі імхо не на 100% інтуїтивний для пересічного юзера
флаффі на десктопі взагалі тормоз і глючний, ідея з флаттером не спрацювала поки що
...
«Фрактал» розвивається, тож недоліки (сподіваюсь) поступово усуватимуться. Натепер мені не подобається:
...
Користуюсь ShcildiChat на телефоні й компʼютері. Основна перевага над Element це можливість відкривати чат на моменті останнього повідомлення.
Ще здається стиль самих повідомлень тут трохи змінений.
Також довгі повідомлення на які відповідають відображаються не повністю, а скорочено з можливістю швидко до них повернутись.
Є й інші покращення, але я давно не використовував Element, тому не памʼятаю чого там немає.

Пробував колись Fluffychat, але мені не сподобався, не зручний.
Також встановлював NeoChat, але на жаль поки що він сироватий. 

шильді ну таке, трохи перефарбований, на десктопі в ньому сенсу мало, шильді некст до речі зараз функціональніший за елемент ікс, там вже є спейси, найс
